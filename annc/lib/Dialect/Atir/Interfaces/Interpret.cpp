...
#include "Dialect/Atir/Interfaces/Interpret.cpp.inc"
...
void AddOp::Interpret() {
  ...
  if (!lhsTensorType || !rhsTensorType) {
    emitError("Expected TensorType for operand, got: ")
            << "lhsType: " << lhsType << " (TensorType: " << isa<atir::TensorType>(lhsType) << ")\n"
            << "rhsType: " << rhsType << " (TensorType: " << isa<atir::TensorType>(rhsType) << ")\n";
    return;
  }
...
void ReluOp::Interpret() {
  ...
  // 
  auto inputTensor = dyn_cast<atir::TensorType>(inputType);
