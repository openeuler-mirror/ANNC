...
namespace atir {
    ...
    // K
    int getKBlockSize(atir::TensorType tensorType, int kidx) {
    ...
    // K
    int64_t getKStart(atir::TensorType tensorType, int kidx, int blockIndex) {
        if (auto tiling = tensorType.getOnchipParallel()) {
    ...
    // K
    int64_t getKSize(atir::TensorType tensorType, int kidx, int blockIndex) {
        if (auto tiling = tensorType.getOnchipParallel()) {
    ...
    Value createLoad(OpBuilder& builder, Location loc, Value input, ArrayAttr axesAtt, ArrayAttr startAtt, ArrayAttr sizeAtt , bool copy) {
        ...
        auto loadOp = builder.create<atir::LoadOp>(loc,outputType, input, axesAtt,startAtt,sizeAtt,BoolAttr::get(builder.getContext(),copy));
    ...
    Value create2DConcat(OpBuilder& builder, Location loc,
        ...
        for (int i = 0; i < blockMCount; i++) {
            ...
            if (rowBlocks.size() > 1) {
                ...
                for (int k = 0; k < rowBlocks.size(); k++) {
                    auto tensorType = llvm::dyn_cast<atir::TensorType>(rowBlocks[k].getType());
        ...
        if (rowResults.size() > 1) {
            ...
            for (int k = 0; k < rowResults.size(); k++) {
                auto tensorType = llvm::dyn_cast<atir::TensorType>(rowResults[k].getType());
    ...
    void unrollMatMul(MatMulOp matmul, BlockInfo blockInfo){
        ...
        auto lhsType = llvm::dyn_cast<atir::TensorType>(matmul.getLhs().getType());
        auto rhsType = llvm::dyn_cast<atir::TensorType>(matmul.getRhs().getType());
    ...
    class AtirUnrollPass : public AtirUnrollBase<AtirUnrollPass> {
    public:
        ...
        void runOnOperation() override {
            ...
            for (auto func : module.getOps<mlir::func::FuncOp>()) {
                func.walk([&](Operation* op) {
                    // onchipParallel
                    if (auto matmul = llvm::dyn_cast<atir::MatMulOp>(op)) {

                        auto lhsType = llvm::dyn_cast<atir::TensorType>(op->getOperand(0).getType());
                        auto rhsType = llvm::dyn_cast<atir::TensorType>(op->getOperand(1).getType());
