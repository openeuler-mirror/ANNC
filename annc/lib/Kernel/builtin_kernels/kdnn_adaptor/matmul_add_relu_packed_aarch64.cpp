...
KernelStatus matmul_add_relu_kdnn_packed_impl(
    ...
    try {
        ...
        KDNN::Gemm gemm(lhs_ref.info, rhs_ref.info, output_ref.info,
                        bias_info, attr);
        // RunWithPackedB consumes the pre-packed rhs layout and applies the
        // relu post-op in the same call.
        gemm.RunWithPackedB(lhs_ref.data, rhs_ref.data, output_ref.data,
                            bias_data, post_op_ptrs);
        return KernelStatus::Success;
    } catch (const std::exception& ex) {
        llvm::errs() << "[ANNC KDNN] Packed MatMulAddRelu failed: "
                     << ex.what() << '\n';
        return KernelStatus::RuntimeError;
    } catch (...) {
        llvm::errs() << "[ANNC KDNN] Packed MatMulAddRelu failed: unknown exception\n";
