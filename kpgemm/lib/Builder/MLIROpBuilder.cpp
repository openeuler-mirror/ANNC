...
void MLIRBuilder::jsonConvertor(const json& graph) {
  ...
  // noneValue_ = builder_.create<pimp::NoneOp>(
...
std::vector<Value> MLIRBuilder::addGraphInput(const json& inputs) {
  ...
  for (int i = 0; i < inputs.size(); i++) {
    ...
    pimp::TensorType inType = getTensorType(name, dtype, shape);
...
#define CREATE_CONSTAN(T, PREC)                                          \
  {                                                                      \
    ...
    auto type = pimp::TensorType::get(shape, builder_.PREC, elems);      \
    auto gOp = builder_.create<pimp::ConstantOp>(                        \
...
void MLIRBuilder::createMatMulOp(const json& node,
  ...
  ArrayRef<NamedAttribute> attrs) {
    ...
    Value C = builder_.create<pimp::BufferOp>(loc, outputTensorType);
    ...
    auto matmul = builder_.create<pimp::MatMulOp>(
      loc,
...
void MLIRBuilder::createAddOp(
  ...
  auto add = builder_.create<pimp::AddOp>(
      loc,
      outs[0],
      ins, // variadic inputs
      builder_.getBoolAttr(do_relu),
...
pimp::TensorType MLIRBuilder::getTensorType(const std::string& name,
  ...
  if (dtype == "float32")
    return pimp::TensorType::get(tensorShape, builder_.getF32Type(),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "uint8")
    return pimp::TensorType::get(tensorShape, builder_.getIntegerType(8),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "int8")
    return pimp::TensorType::get(tensorShape, builder_.getI8Type(),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "int16")
    return pimp::TensorType::get(tensorShape, builder_.getI16Type(),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "int32")
    return pimp::TensorType::get(tensorShape, builder_.getI32Type(),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "int64")
    return pimp::TensorType::get(tensorShape, builder_.getI64Type(),
                                 builder_.getStringAttr(name), encoding);
  if (dtype == "float16")
    return pimp::TensorType::get(tensorShape, builder_.getF16Type(),
                                 builder_.getStringAttr(name), encoding);
  llvm::report_fatal_error("Unsupported data type");
}
}  // namespace kpgemm