...
#define INTERPRET_SINGLE_OP(OpType, SrcOp)            \
  if (llvm::isa<pimp::OpType>(SrcOp)) {               \
    auto kp_op = llvm::dyn_cast<pimp::OpType>(SrcOp); \
    kp_op.Interpret();                                \
    break;                                            \
  }
...
bool runKpGenOP(const std::string &libPath, IoTensorDef *inputs,
  ...
  for (const auto &tensorData : inputs->getInputs()) {
    ...
    if (auto tensorType = dyn_cast<pimp::TensorType>(type)) {
      // shapevector
      mlir::ArrayRef<int64_t> shape = tensorType.getShape();
  ...
  // pimp::TensorTypecacheData
  auto tensorType = pimp::TensorType::get({M, N}, f32Type);
...
void runOriGraph(mlir::ModuleOp *root, const std::string &funcName,
  ...
  targetFuncOp.walk([&](mlir::Operation *op) {
    ...
    bool skip = llvm::TypeSwitch<mlir::Operation *, bool>(op)
                    ...
                    .Case<pimp::NoneOp>([](auto) { return true; })
...
bool verifyTensor(IoTensorDef *lhs, IoTensorDef *rhs, double tolerance = 1e-6) {
  ...
  for (size_t i = 0; i < lhsCount; ++i) {
    ...
    try {
      ...
      auto lhsTensorType = dyn_cast<pimp::TensorType>(lhsOpResult.getType());
      auto rhsTensorType = dyn_cast<pimp::TensorType>(rhsOpResult.getType());
...
namespace pimp {
class PimpOpVerifyPass : public PimpOpVerifyBase<PimpOpVerifyPass> {
 public:
  PimpOpVerifyPass() = default;
  ...
  void runOnOperation() override {
    ...
    if (funcName.empty()) {
      emitError(getOperation().getLoc(),
                ...
                "--pimp-op-verify=\"kpGenLibPath=/path/to/lib.so\"");
      signalPassFailure();
      return;
    }
...
std::unique_ptr<OperationPass<ModuleOp>> createPimpOpVerifyPass() {
  return std::make_unique<PimpOpVerifyPass>();
}
}  // namespace pimp