...
namespace pimp {
    ...
    public:
        LogicalResult matchAndRewrite(ReluOp op,
            ...
            if (llvm::isa<pimp::AddOp>(reluSourceOp)) {
                auto addOp = llvm::dyn_cast<pimp::AddOp>(reluSourceOp);
                ...
                auto newAddOp = rewriter.create<pimp::AddOp>(op.getLoc(), outs,
            ...
            for (auto* user: op.getResult().getUsers()) {
                if (auto addOp = llvm::dyn_cast<pimp::AddOp>(user)) {
                    ...
                    if (addOp.getOperand(1).getDefiningOp() != nullptr
                        && llvm::isa<pimp::ConstantOp>(addOp.getOperand(1).getDefiningOp())) {
                        addOp.getOperand(1).getDefiningOp()->moveBefore(op);
                    }

                    auto newMatmulOp = rewriter.create<pimp::MatMulOp>(addOp.getLoc(), outs,
                                                    ins,attrs);
    ...
    class PimpBlockFusionPass : public PimpBlockFusionBase<PimpBlockFusionPass> {
    public:
        PimpBlockFusionPass() = default;

        void runOnOperation() override {
            std::cout << "this is PimpBlockFusionPass" << std::endl;
    ...
    std::unique_ptr<OperationPass<ModuleOp>> createPimpBlockFusionPass() {
        std::cout << "this is createPimpBlockFusionPass" << std::endl;
        return std::make_unique<PimpBlockFusionPass>();
    }
}  // namespace pimp