...
from .dialects import pimp, func
...
class ForLoop:
    ...
    def __call__(self, lower: int, 
        ...
        with mlir.Location.unknown():
            ...
            loop_op = pimp.ForOp([], 0, [], start=lower, end=upper, step=step, axis=axis, 
    ...
    @property
    def loop(self) -> pimp.ForOp:
        return self.loop_ops[-1]
    
    def clone_new_loop(self, ret_types: List[mlir.Type], ip: mlir.InsertionPoint = None) -> pimp.ForOp:
        with mlir.Location.unknown():
            loop = pimp.ForOp(ret_types, 0, [], start=self.loop.start, end=self.loop.end, 
            ...
            with mlir.InsertionPoint(block):
                ...
                for op in origin_block.operations:
                    if isinstance(op, pimp.ReturnOp):
                        has_return = True
...
class Pimp:
    ...
    def none(self) -> Value:
        return get_value_ptr(pimp.NoneOp(
    ...
    def matmul(self, output: TensorType, lhs: Value, rhs: Value, bias: Value = None) -> Value:
        ...
        return get_value_ptr(pimp.MatMulOp(
    ...
    def add(self, output: TensorType, lhs: Value, rhs: Value, scalar: int = None) -> Value:
        return get_value_ptr(pimp.AddOp(wrap_type(output), lhs.value, rhs.value, scalar, 
                                       loc=self.Loc.unknown(), ip=state.ip).output)

    def relu(self, output:TensorType, input: Value):
        return get_value_ptr(pimp.ReluOp(wrap_type(output), input.value,
                             loc=self.Loc.unknown(), ip=state.ip).output)

    def ret(self, ret_vals: List[Value]):
        pimp.ReturnOp([val.value for val in ret_vals], loc=self.Loc.unknown(), ip=state.ip)


def none(loc=None, **kwargs):
    loc = loc or mlir.Location.unknown()
    return Value(pimp.NoneOp(mlir.NoneType.get(), loc=loc, ip=state.ip).output)


def ret(values: List[Value], loc=None) -> List[Value]:
    loc = loc or mlir.Location.unknown()
    op = pimp.ReturnOp([i.value for i in values], loc=loc, ip=state.ip)
...
def constant(name: str = None, dtype: ElemType = None, shape: List[int] = None,
    ...
    if dtype is not None and shape is not None:
        return Value(pimp.ConstantOp(wrap_type(TensorType.get(shape, dtype())),
                                     name, sym_visibility=sym_visibility,
                                     loc=loc, ip=state.ip).data)
    if data is not None:
        return Value(pimp.ConstantOp(wrap_type(TensorType.get_data(data)),
                                     name, sym_visibility=sym_visibility,
                                     loc=loc, ip=state.ip).data)
...
def matmul(dtype: ElemType, shape: List[int], lhs: Value, rhs: Value, C: Value , withBias: bool, loc=None, bias_add: Value = None, **kwargs) -> Value:
    ...
    return Value(pimp.MatMulOp(wrap_type(TensorType.get(shape, dtype, **kwargs)),
...
def add(dtype: ElemType, shape: List[int], operands: List[Value], do_relu: bool = False, relu_limit: float = -1.0, scalar: int = None, loc=None, **kwargs) -> Value:
    ...
    return Value(pimp.AddOp(wrap_type(TensorType.get(shape, dtype, **kwargs)),
...
def relu(dtype: ElemType, shape: List[int], input: Value, relu_limit: float = 0.0, loc=None, **kwargs) -> Value:
    loc = loc or mlir.Location.unknown()
    return Value(pimp.ReluOp(wrap_type(TensorType.get(shape, dtype, **kwargs)), 
