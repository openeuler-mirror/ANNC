...
// TF control-flow mirror ops: the mirror verifiers keep data/output types
// identical, so the pass never sees mixed states inside an op. Its role is
// cross-op propagation: a consumer-side rank pin on one side must flow
// through the mirror op to the data input and sibling outputs.
